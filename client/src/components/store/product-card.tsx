    ...
    >
      <div className="flex flex-col h-full">
        ...
        <div className="px-2 pt-1.5 pb-1">
          <p className="text-[8px] font-medium text-muted-foreground uppercase tracking-wider mb-0.5" data-testid={`text-category-${product.id}`}>{productCategory}</p>
        ...
        <div className="relative flex-1 px-2 py-1 min-h-0">
          <div className="relative w-full h-full rounded-lg overflow-hidden bg-gray-50">
            ...
            {activeDiscount && (
              <motion.div 
                ...
                className="absolute top-2 left-2 bg-gradient-to-br from-red-500 to-red-600 text-white px-2 py-1 rounded-full text-xs font-black shadow-lg shadow-red-500/50 border border-yellow-300" 
        ...
        {hasVariants && (
          <div className="px-2 py-1 flex gap-2 overflow-x-auto no-scrollbar flex-shrink-0">
            {product.units && product.units.length > 0 && (
        ...
        <div className="px-2 py-1.5 border-t border-gray-100 flex items-center justify-between gap-1">
          <div className="flex flex-col gap-0">
            {activeDiscount ? (
              <>
                <p className="font-bold text-xs text-green-600" data-testid={`text-price-${product.id}`}> L.E {discountedPrice.toFixed(2)}</p>
                <p className="text-[10px] text-gray-400 line-through"> L.E {product.price.toFixed(2)}</p>
              </>
            ) : (
              <p className="font-bold text-xs" data-testid={`text-price-${product.id}`}> L.E {product.price.toFixed(2)}</p>
            )}
          </div>
          <div className="flex items-center gap-1">
            {!isAvailable && (
              <p className="text-[9px] font-semibold text-red-600 bg-red-50 px-1 py-0.5 rounded whitespace-nowrap" data-testid={`text-unavailable-${product.id}`}>{t("unavailable", language)}</p>
            )}
            {isAvailable && (
              <p className="text-[9px] font-semibold text-green-600 bg-green-50 px-1 py-0.5 rounded whitespace-nowrap" data-testid={`text-available-${product.id}`}>{t("available", language)}</p>
            )}
            <button
              onClick={handleAddToCart}
              disabled={isAdding}
              className="w-7 h-7 bg-black text-white rounded-full shadow-md flex items-center justify-center hover:bg-gray-900 transition-colors disabled:opacity-50 flex-shrink-0"
