  ...
  return (
    <MobileWrapper>
      <div className="w-full flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <div className="px-4 pb-4 pt-2 flex items-center justify-between gap-4 border-b border-gray-100 flex-shrink-0">
        ...
        {/* Tabs */}
        {user && (
          <div className="flex gap-0 border-b border-gray-100 flex-shrink-0 px-4 bg-white">
            <button
                        ...
                        {newItemForm.units.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {newItemForm.units.map((unit, i) => (
                              <span key={i} className="inline-flex items-center gap-1 px-4 py-1 bg-blue-100 text-blue-700 rounded text-xs">
                                {unit}
                        ...
                        {newItemForm.sizes.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {newItemForm.sizes.map((size, i) => (
                              <span key={i} className="inline-flex items-center gap-1 px-4 py-1 bg-green-100 text-green-700 rounded text-xs">
                                {size}
                        ...
                        {newItemForm.colors.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {newItemForm.colors.map((color, i) => {
                              ...
                              return (
                                <span key={i} className="inline-flex items-center gap-1 px-4 py-1 bg-red-100 text-red-700 rounded text-xs border" style={{borderColor: colorHex}}>
                            ...
                            <div className="flex-1 flex gap-3">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {item.units && item.units.map((u: string) => <span key={u} className="inline-block px-4 py-1 bg-blue-100 text-blue-700 rounded text-xs">{u}</span>)}
                                  {item.sizes && item.sizes.map((s: string) => <span key={s} className="inline-block px-4 py-1 bg-green-100 text-green-700 rounded text-xs">{s}</span>)}
                                  {item.colors && item.colors.map((c: string) => {
                                    ...
                                    return (
                                      <span key={colorName} className="inline-block px-4 py-1 bg-red-100 text-red-700 rounded text-xs flex items-center gap-1 border" style={{borderColor: colorHex}}>
                                        <span style={{width: '8px', height: '8px', backgroundColor: colorHex, borderRadius: '2px'}}></span>
                                        {colorName}
                                      </span>
                                    );
                                  })}
                                  {!item.available && <span className="inline-block px-4 py-1 bg-red-100 text-red-700 rounded text-xs">{t("notAvailable", language)}</span>}
                                </div>
                              </div>
                            </div>
