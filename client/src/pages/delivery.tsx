      ...
      {isLoading ? (
        ...
        <div className="flex flex-col h-screen relative">
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            {/* Map View */}
            {viewMode === "map" && (
              <div className="flex-1 overflow-hidden flex flex-col px-5 py-2 pb-32 gap-2">
                {mapError ? (
                  ...
                  <>
                    ...
                    <div className="p-2 bg-gray-50 rounded-2xl border border-gray-200 flex-shrink-0">
                      <div className="grid grid-cols-2 gap-2 mb-2">
                        <div>
                          <p className="text-xs text-gray-600 font-semibold">{language === "ar" ? "المسافة" : "Distance"}</p>
                          <p className="text-lg font-bold text-orange-600 mt-0.5">{totalDistance.toFixed(1)} <span className="text-xs">km</span></p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-600 font-semibold">{language === "ar" ? "الوقت" : "Time"}</p>
                          <p className="text-lg font-bold text-blue-600 mt-0.5">{Math.round((totalDistance / selectedSpeed) * 60)} <span className="text-xs">{language === "ar" ? "د" : "m"}</span></p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => setSelectedSpeed(30)} className={`flex-1 py-1.5 px-2 rounded-lg font-semibold text-sm ${selectedSpeed === 30 ? "bg-orange-500 text-white" : "bg-white text-gray-700 border border-gray-200"}`} data-testid="button-speed-30">30</button>
                        <button onClick={() => setSelectedSpeed(50)} className={`flex-1 py-1.5 px-2 rounded-lg font-semibold text-sm ${selectedSpeed === 50 ? "bg-orange-500 text-white" : "bg-white text-gray-700 border border-gray-200"}`} data-testid="button-speed-50">50</button>
                        <button onClick={() => setSelectedSpeed(70)} className={`flex-1 py-1.5 px-2 rounded-lg font-semibold text-sm ${selectedSpeed === 70 ? "bg-orange-500 text-white" : "bg-white text-gray-700 border border-gray-200"}`} data-testid="button-speed-70">70</button>
                      </div>
                    </div>
                  </>
                )}
